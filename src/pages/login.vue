...
export default {
  ...
  methods: {
    login() {
      ...
      this.axios
        .post("/user/login", {
          username, //username是username：username，参数一样的简写
          password,
        })
        .then((res) => {
          // this.res = res;//看反回结果用的
          this.$cookie.set("userId", res.id, { expires: "Session" }); //过期时间Session会话级别，当浏览器关掉并退出就过期；
          //this.$store.dispatch('saveUserName', res.username);//保存用户名与mapActions方式二选一
          this.saveUserName(res.username);
          // this.$router.push('/index') //登陆成功跳到首页
          this.$router.push({
            //name +  params
            name: "index", //对应router.js里的页面name
            params: {
              from: "login",
            },
          });
          /* 
        this.$router.push({//path+query,url会一直有"?from=login"，不推荐用；
          path:'/index',
          query:{
            from:'login'
           }
